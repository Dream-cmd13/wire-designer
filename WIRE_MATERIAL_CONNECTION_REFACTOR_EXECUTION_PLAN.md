...
信息窗按实际连接器动态生成列，不再固定使用“左侧 PIN / 右侧 PIN”：

```text
线材 | A端（JST XH 2P） | B端（JST XH 4P） | ... | 颜色 | 接线定义
W1   | Pin1              | Pin3              |     | 红色 | SIG
```

规则：

- 每连接一个新的连接器，就在颜色列之前增加一个以连接器标签和型号命名的 PIN 列；
- PIN 单元格只接受 `Pin+数字` 或空白，空白表示解除该端点绑定；
...
- 删除线材时，从保护套 `attachedMaterialIds` 中移除该线材，其他覆盖关系继续保留；
...
- 保护套的每个 `attachedMaterialIds` 是否存在；
- 所有 ID 是否唯一。
...
1. 新建一条红色电子线；
2. 仅将左端接到连接器左侧 Pin2；
3. 信息窗立即增加以该连接器标签和型号命名的列，并显示 `Pin2`、红色和 SIG；
4. 不生成“左侧 PIN / 右侧 PIN”固定列；
5. 保存并重新打开项目，状态不丢失。

### B. 反方向单端连接

1. 仅将线材右端接到连接器右侧 Pin4；
2. 信息窗在对应连接器动态列中显示 `Pin4`；
3. 颜色和 SIG 正常显示并可编辑。
...
1. **护套线 UL 号可选项有哪些？**  
   当前只明确“单选、可无”，但没有给出允许选择的 UL 号目录。
   答：选项如下 `UL2464`、`UL20276` 
...
## 15. 2026-07-02 线材分组与附件交互扩展

本轮在 v3 模型上继续增加以下已实施规则：

1. 多条电子线只要共享任一连接器，就组成同一个显示组；
2. 合并线材信息窗口只显示在组内纵坐标最下方的电子线下面；
3. 合并信息窗口严格按线材一行展示，连接器列位于颜色列之前；
4. 连接器列标题使用连接器实例标签，并同时显示连接器型号；
5. 保护套通过 `attachedMaterialIds: string[]` 绑定线材，可覆盖一条线、四条线或四条中的任意两条；
6. 保护套高度根据所选线材的垂直范围自动计算，线材移动或改长后重新定位；
7. 护套线的连接边使用实线；除拖拽连接外，也支持依次点击连接器 PIN 点和线材端点；
8. 信息窗口中的 PIN 可直接修改，只允许 `Pin+数字` 或空白；
9. 更换连接器型号后保持连接器 selection，左侧属性编辑器不关闭；
10. 画布右键菜单增加“添加模型”，当前提供 1 个可选“方块外模”，放置后显示在线材与连接器之间，并遮住被包覆的中间连接线；
11. 线材右键菜单增加“添加标签”和“添加号码管”；
12. 标签固定材质为“五防热敏纸标签纸”，保存内容和长度；号码管保存内容和长度。
