...
import { clearDrawingCanvas, moveDrawingLayers, splitDrawingObjects, toggleDrawingLocks } from '@/lib/drawingCommands';
...
export function DrawingWorkbenchPage() {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Escape') { setToolMode('select'); setSelectedObjectIds([]); }
      ...
      if (event.shiftKey && key === 'w') { event.preventDefault(); setToolMode('line'); }
      if (event.shiftKey && key === 'e') { event.preventDefault(); setToolMode('polyline'); }
      if (event.shiftKey && key === 'r') { event.preventDefault(); setToolMode('curve'); }
    };
  ...
  return <div className="flex h-full min-h-0 flex-col bg-slate-100">
    <DrawingWorkbenchToolbar toolMode={toolMode} orthogonal={orthogonal} hasSelection={selected.length > 0} selectionLocked={selected.some((object) => object.locked)} canUndo={past.length > 0} canRedo={future.length > 0} onWizard={() => setWizardOpen(true)} onResources={() => setResourcesOpen((value) => !value)} onUndo={undo} onRedo={redo} onClear={clear} onDelete={removeSelected} onToggleLock={toggleLocks} onLayer={moveLayers} onToolMode={setToolMode} onOrthogonal={() => setOrthogonal((value) => !value)} onAddText={() => addResource('text')} onAddLabel={() => addResource('label')} onAddDimension={() => addResource('dimension')} onAddTable={() => addResource('table')} onSave={markSaved} onPdf={() => downloadDrawingPdf(drawing)}/>
    <div className="relative flex min-h-0 flex-1">
      ...
      <StandaloneDrawingCanvas drawing={drawing} selectedObjectId={primaryId} selectedObjectIds={selectedObjectIds} zoom={zoom} toolMode={toolMode} orthogonal={orthogonal} onSelectObject={(id) => { if (!id) setSelectedObjectIds([]); else if (!selectedObjectIds.includes(id)) setSelectedObjectIds([id]); }} onSelectionChange={setSelectedObjectIds} onStartEdit={remember} onUpdateObject={updateObject} onAddObject={addObject}/>
      <StandaloneDrawingInspector drawing={drawing} selectedObjectId={primaryId} onStartEdit={remember} onUpdateObject={updateObject}/>
    </div>
