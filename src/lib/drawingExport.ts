...
function svgObject(object: DrawingObject): string {
  ...
  if (object.kind === 'table' || object.kind === 'bom-table' || object.kind === 'wiring-table') {
    ...
    const text = (key: string, value: string, x: number, baseline: number, fallbackSize: number, cellWidth: number, centered = false) => {
      ...
      const fitAttributes = fontSize < configuredFontSize
        ...
        : '';
      const textAnchor = centered ? ' text-anchor="middle"' : '';
      const textX = centered ? x + cellWidth / 2 : x;
      return `<text x="${textX + offset.x}" y="${baseline + offset.y}" font-size="${fontSize}"${textAnchor}${fitAttributes} fill="${object.style.color}" stroke="none">${escapeXml(value)}</text>`;
    };
    ...
    const cells = resolveDrawingTableCells(object).map((cell) => {
      const projection = object.projectionCellKey === cell.key
        ...
        : text(cell.key, cell.value, cell.x, cell.y + cell.height - 6, cell.header ? object.style.fontSize : Math.max(8, object.style.fontSize - 1), cell.width, true);
