...
export function StandaloneDrawingInspector({ drawing, selectedObjectId, onStartEdit, onUpdateObject }: StandaloneDrawingInspectorProps) {
  ...
  return (
    <aside className="w-72 shrink-0 overflow-y-auto border-l border-slate-200 bg-white p-4">
      ...
      {(object.kind === 'table' || object.kind === 'bom-table' || object.kind === 'wiring-table') && <label className="mt-4 block text-xs font-medium text-slate-600">表格标题<input disabled={readonly} value={object.title} onChange={(event) => update({ title: event.target.value } as Partial<DrawingObject>)} className="mt-1 w-full rounded border border-slate-300 px-2 py-1.5 text-sm" /></label>}
      {object.kind === 'tech-requirements' && <label className="mt-4 block text-xs font-medium text-slate-600">技术要求<textarea disabled={readonly} value={object.requirements.join('\n')} onChange={(event) => update({ requirements: event.target.value.split('\n').filter(Boolean) } as Partial<DrawingObject>)} className="mt-1 min-h-28 w-full rounded border border-slate-300 p-2 text-sm" /></label>}
