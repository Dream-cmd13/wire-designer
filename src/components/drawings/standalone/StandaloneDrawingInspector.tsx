...
interface Props {
  ...
  onUpdateObjects?: (objectIds: string[], patch: Partial<DrawingObject>) => void;
...
export function StandaloneDrawingInspector({ drawing, selectedObjectId, selectedObjectIds, onStartEdit, onUpdateObject, onUpdateObjects, onSetLayer }: Props) {
  ...
  const update = (patch: Partial<DrawingObject>) => {
    onStartEdit();
    if (multiple && onUpdateObjects) onUpdateObjects(selectionIds, patch);
    else onUpdateObject(object.id, patch);
  };
  const readonly = selection.some((candidate) => candidate.locked);
