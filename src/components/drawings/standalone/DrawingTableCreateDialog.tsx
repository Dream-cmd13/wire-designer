...
export function DrawingTableCreateDialog({ open, onConfirm, onClose }: Props) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-900/20 p-4 backdrop-blur-[1px]" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <form role="dialog" aria-modal="true" aria-labelledby="table-create-title" onSubmit={(event) => { event.preventDefault(); if (valid) onConfirm({ rowCount, columnCount, showTitleRow }); }} className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-5 shadow-2xl">
      <div className="flex items-start gap-3">
        ...
        <button type="button" aria-label="关闭表格创建窗口" onClick={onClose} className="rounded-md p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600"><X className="h-4 w-4"/></button>
      </div>
