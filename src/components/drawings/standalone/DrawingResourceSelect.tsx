...
export function DrawingResourceSelect({ title, resources, filters, selectedId, loading, error, onFiltersChange, onSelect, onRetry }: Props) {
  ...
  return <section className="rounded border border-slate-200 p-3">
    ...
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <label className="text-xs text-slate-600">类别<select className={field} value={filters.category ?? ''} onChange={(event) => patch({ category: event.target.value || undefined })}><option value="">全部</option>{values('category').map((value) => <option key={value}>{value}</option>)}</select></label>
    ...
    <div className="mt-3 grid max-h-44 gap-2 overflow-y-auto sm:grid-cols-2">
      {resources.map((resource) => <button type="button" key={resource.catalogItemId} onClick={() => onSelect(resource)} className={`rounded border p-2 text-left text-xs ${selectedId === resource.id || selectedId === resource.catalogItemId ? 'border-blue-500 bg-blue-50' : 'border-slate-200 hover:bg-slate-50'}`}><strong className="block text-slate-800">{resource.name}</strong><span className="text-slate-500">{resource.model} · {resource.pinCount ?? '-'}PIN · {resource.rowCount ?? '-'}排 · {resource.pitchMm ?? '-'}mm</span></button>)}
    </div>
  </section>;
}
