...
> 本文保留初期实施方案。成本分析接入后的现行价格规则以[成本分析文档的售价与最低售价来源规则](./finished-harness-cost-analysis-plan.md#售价与最低售价来源规则2026-09-18-确认)为准：CRM `son_price_low` 仅显示为“最低售价”；Excel `sales_price` 仅显示为“售价”。列表、详情及复制报告独立展示两者，禁止互相兜底；下文初期“成本分析/报价暂无”的占位设计不再代表当前实现。
...
| `packing` | 包装规格 |
| `son_unit` | 物料单位 |
| `son_price_low` | CRM 平台最低售价，仅由 CRM 导入，不得用 Excel 售价回填 |
